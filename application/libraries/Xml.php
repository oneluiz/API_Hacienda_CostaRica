 ...
 * Extends XMLWriter (PHP5 or higher!)
 * 
 ...
 * $this->xml->startBranch('car', array('country' => 'usa', 'type' => 'racecar'));
 * 
 * End (close) a branch
 * $this->xml->endBranch();
 * 
 * Add a CDATA node with attributes:
 * $this->xml->addNode('model', 'Corolla', array('year' => '2019'), true);
 * 
 * Print the XMl directly to screen:
 * $this->xml->getXml(true);
 * 
 * Pass the XMl to a view file:
 * $data['xml'] = $this->xml->getXml();
 * $this->load->view('xml_template', $data);
 * 
 * @name /library/Xml.php
 * @category Accent_application
 * @version 1.1
 * @author Joost van Veen
 * @author Luis Cortés Juárez
...
class Xml extends XMLWriter
{
    ...
    /**
     * Name of the attributes element for this XMl. Defaults to attributes.
     */
    private $_attributes = '';
    ...
    /**
     * Set the value of the attributes for root tag for this XML
     */
